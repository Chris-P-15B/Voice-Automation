...
# Checks NumPlan for CFA, CFB, CFNA, CFNC, CFUR, AAR Destination Mask or Called Party Transformation that reference a given number, SQL wildcard % can be used
...
# GUI and main code
class NumberReferenceFinderFrame(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        tk.Button(self, text="Find References", command=self.find_references).place(relx=0.35, rely=0.12, height=22, width=100)
    ...
    def read_axl(self, search_string):
        ...
        sql_statement = "SELECT n.DNOrPattern, n.Description, n.tkPatternUsage FROM NumPlan n LEFT JOIN CallForwardDynamic cfd ON cfd.fkNumPlan=n.pkid WHERE n.CFAptDestination LIKE '" \
            + search_string + \
            "' OR n.CFBDestination LIKE '" \
            + search_string + \
            "' OR n.CFBIntDestination LIKE '" \
            + search_string + \
            "' OR n.CFNADestination LIKE '" \
            + search_string + \
            "' OR n.CFNAIntDestination LIKE '" \
            + search_string + \
            "' OR n.PFFDestination LIKE '" \
            + search_string + \
            "' OR n.PFFIntDestination LIKE '" \
            + search_string + \
            "' OR n.CFURDestination LIKE '" \
            + search_string + \
            "' OR n.CFURIntDestination LIKE '" \
            + search_string + \
            "' OR n.AARDestinationMask LIKE '" \
            + search_string + \
            "' OR n.CalledPartyTransformationMask LIKE '" \
            + search_string + \
            "' OR cfd.CFADestination LIKE '" \
            + search_string + \
        ...
        try:
            for row in self.sql_query(service=axl, sql_statement=sql_statement):
                try:
                    ...
                    self.list_box.insert(tk.END, n_dnorpattern + ' "' + n_description + '", ' + self.pattern_usage[n_tkpatternusage])
                    cntr += 1
                except TypeError:
                    continue
        except TypeError:
            ...
            return
        self.records_label_text.set("Dial Plan Records: " + str(cntr))
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title("Number Reference Finder v1.1")
    NumberReferenceFinderFrame(root)
    root.mainloop()