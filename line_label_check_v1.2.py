...
# GUI and main code
class LineTextCheckFrame(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        tk.Button(self, text="Check Line Labels", command=self.check_labels).place(relx=0.1, rely=0.12, height=22, width=120)
        tk.Button(self, text="Update Line Labels", command=self.update_labels).place(relx=0.5, rely=0.12, height=22, width=120)
    ...
    def read_axl(self, output_filename):
        ...
        cntr = 0
        result_list = [["Device Name", "DN", "Alerting Name", "Display Name", "Line Text Label", "New Line Label", "pkid"]]
        self.list_box.insert(tk.END, "Device Name, DN, Alerting Name, Display Name, Line Text Label, New Line Label, pkid\n")
        sql_statement = "SELECT d.name, n.dnorpattern, n.alertingname, dnmap.display, dnmap.label, dnmap.pkid FROM device d INNER JOIN devicenumplanmap dnmap ON dnmap.fkdevice=d.pkid INNER JOIN numplan n ON dnmap.fknumplan=n.pkid WHERE (d.tkclass=1 OR d.tkclass=254) ORDER BY d.name"
        try:
            for row in self.sql_query(service=axl, sql_statement=sql_statement):
                try:
                    ...
                    if n_dnorpattern not in dnmap_label:
                        ...
                        if len(name_words) > 1:
                            new_label += name_words[0][0] + ' '
                            new_label += name_words[-1]
                            new_label += '-' + n_dnorpattern
                        elif len(name_words) == 1:
                            new_label += name_words[0] + "-" + n_dnorpattern
                        new_label = new_label.replace("'", "")
                        self.list_box.insert(tk.END, d_name + ', ' + n_dnorpattern + ', ' + n_alertingname + ', ' + dnmap_display + ', ' + dnmap_label + ', ' + new_label + ', ' + dnmap_pkid)
                        result_list.append([d_name, n_dnorpattern, n_alertingname, dnmap_display, dnmap_label, new_label, dnmap_pkid])
                        cntr += 1
                except TypeError:
                    continue
        except TypeError:
    ...
    def write_axl(self, output_filename):
        ...
        try:
            with open(self.csv_input_filename, encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    try:
                        sql_statement = "UPDATE devicenumplanmap SET label='" + row[5].replace("'", "") + "' WHERE pkid='" + row[6] + "'"
